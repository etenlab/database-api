...
export class CronService {
  ...
  constructor(
    ...
    private pbLangDetailService: ProgressBibleLanguageDetailsService,
  ) {}
  ...
  async handleCron() {
    ...
    var sharedSecret =
      '22d04c4562fa4492f382aa914b04a120ddc85d63cc357e39033919846fd2c32747c99387d7c1c30d90624a625ceb43e43dcb143ab5f3378d9e1449a59ee7af15'; // insert Key Secret received from API Developer Portal.

    var unixTimestamp = Math.floor(Date.now() / 1000);
    var hmac = createHmac('sha1', sharedSecret);
    var signature = hmac.update(unixTimestamp + apiKey).digest('hex');
    var url =
      'https://progressbible-language-v1.apis.sil.org?api_key=' +
      apiKey +
      '&api_sig=' +
      signature;
    ...
    result.forEach((value) => {
      ...
      if (value.status == 200) {
        value.data.resource.forEach(async (entry: any) => {
          var count = await this.pbLangDetailService.isExist(
            entry.UnitCode,
            entry.PrimaryCountryCode,
          );
          // console.log(count);
          if (!count) {
            // console.log(entry);
